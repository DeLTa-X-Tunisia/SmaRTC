...
class SmaRTCClient {
    constructor(config) {
        ...
        this.connection = new signalR__namespace.HubConnectionBuilder()
            ...
            .withHubProtocol(new signalR__namespace.JsonHubProtocol()) // MessagePack would be: .withHubProtocol(new signalR.MessagePackHubProtocol())
    ...
    async disconnect() {
        // Leave session
        if (this.connection.state === signalR__namespace.HubConnectionState.Connected) {
    ...
    async sendSignal(targetPeerId, signal) {
        ...
        if (peer?.dataChannel?.readyState === 'open') {
            const encoded = msgpack.encode(signal);
    ...
    setupDataChannel(peerId, channel) {
        ...
        channel.onmessage = (event) => {
            try {
                const signal = msgpack.decode(new Uint8Array(event.data));
